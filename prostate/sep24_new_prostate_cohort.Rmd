...
# Full analyses
```{r}
coverage_fn <- list.files("/project/data/", pattern = "^RESEQ.*", full.names = T)
coverage_list <- lapply(coverage_fn,read_xlsx, col_types = "text")
names(coverage_list) <- gsub("_TotalCov.*","",basename(coverage_fn))
#coverage_list <- lapply(coverage_list,setNames, colnames(coverage_list[[1]])) # old format
...
# Add missing coverage for this run
coverage_df <- coverage_df %>%
...
sample_sheet <- sample_sheet %>%
  ...
  filter(!is.na(`Sequencing successful (Y/N)`) | sampleID %in% coverage_df$sampleID) 
...
write.csv(run_sample_sheet, "/project/results/tumor_normal_RESEQ27_to_RESEQ34.csv",
          row.names=F)

not_sequenced <- sample_sheet %>%
  filter(`Sequencing successful (Y/N)` == "N")
