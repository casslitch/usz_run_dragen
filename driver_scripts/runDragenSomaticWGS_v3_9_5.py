...
def params():
    ...
    parser.add_argument('-s','--sampleSheet',
                        required = True,
                        help = 'SampleSheet used during the bcl2fastq coversion')
    '''
...
def createFastqListFile(sampleName,bclFolder,inputDir, outDir, runIDs=None):
    outFilePath = outDir + sampleName + '_fastqList.csv'
    output = [['RGID','RGSM','RGLB','Lane','Read1File','Read2File']]
    if runIDs is None:
        runList = glob(inputDir + '2*_A00854_*')
    else:
        runIDList=[id for id in runIDs.split(',')]
        runList = [f for f in glob(inputDir + '*_A00854_*') if os.path.basename(f) in runIDList]
    for run in runList:
...
def runDragenNormal(sampleName,sampleFastqListFile,outDir):
    ...
    if not os.path.isfile(sampleOutDir + '/' + sampleName + '.mapping_metrics.csv'):
        ...
        call('dragen \
            ...
            --trim-adapter-read1=/home/agudo/wgs_adapters/adapter_read1.fa \
            --trim-adapter-read2=/home/agudo/wgs_adapters/adapter_read2.fa \
            ...
            --variant-annotation-data /mnt/MTP_Storage_SSD/hg38_databases/nirvana_hg38_data/ \
...
def runDragenTumorNormal(sampleName,tumorFastqListFile,normalFastqListFile,normalOutDir,outDir):
    ...
    call('dragen \
             ...
             --enable-variant-caller true --vc-enable-joint-detection true \
             --panel-of-normals /mnt/WGS/PON/pon.vcf.gz \
             --enable-sv true \
             --qc-somatic-contam-vcf /opt/edico/config/somatic_sample_cross_contamination_resource_hg38.vcf.gz \
             --vc-systematic-noise /mnt/MTP_Storage_SSD/genome/systematic-noise-baseline-collection-1.0.0/WGS_hg38_v1.0_systematic_noise.bed.gz \
             --enable-tmb true \
             ...
             --enable-hrd true \
             --enable-hla=true --hla-min-reads=50 \
...
def main():
    ...
    for sampleName in sampleMatch:
        ...
        try:
            ...
            normalFastqListFile = createFastqListFile(normalSample,args.bclFolder,args.inputDir, args.outDir)
            if not args.germlineOnly:
                tumorFastqListFile = createFastqListFile(sampleName,args.bclFolder,args.inputDir, args.outDir, runIDs=args.runIDsTumor)
            if not args.dryRun:
